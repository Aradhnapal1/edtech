...
<script>
    ...
    document.getElementById('exportOptions').addEventListener('change', function () {
        ...
        table.querySelectorAll('tbody tr').forEach(tr => {
            ...
            tr.querySelectorAll('td').forEach((td, index) => {
                // For the image + name column, we only take the text (student name)
    ...
    new DataTable('#studentTable', {
        ...
        paging: false,
        "columnDefs": [
            { "orderable": false, "targets": [0, 5, 6,4,3,2,1] } // Disable sorting on S.No., Edit, Delete columns
        ]
    });

    if (typeof ensureDtColumnOrderSpans === 'function') {
        ensureDtColumnOrderSpans('#studentTable');
    }
</script>
</body>
</html>