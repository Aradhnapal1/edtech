...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Ensure DataTables column order span is present so CSS glyphs show
  function ensureDtColumnOrderSpans(selector) {
    try {
      const table = document.querySelector(selector);
      if (!table) return;
      table.querySelectorAll("thead th").forEach((th) => {
        // Only add for sortable headers (DataTables adds 'sorting_disabled' to non-sortable)
        if (th.classList.contains("sorting_disabled")) return;
        if (!th.querySelector(".dt-column-order")) {
      ...
      // fail silently
      console.debug("ensureDtColumnOrderSpans error", e);
    }
  }

  // expose globally so page-specific inline scripts can call it after initializing DataTable
  window.ensureDtColumnOrderSpans = ensureDtColumnOrderSpans;
